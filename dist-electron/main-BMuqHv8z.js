...
ipcMain.handle("assistant:run", async (_event, {
  ...
  const { GoogleGenAI } = await import("./index-C75ZjSfH.js");
...
ipcMain.handle("window:setFloatingMode", async (_event, mode) => {
...
ipcMain.handle("window:setCompact", async (_event, flag) => {
  if (!win) return { ok: false };
  if (flag) {
